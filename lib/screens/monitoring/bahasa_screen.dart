...
class _BahasaScreenState extends State<BahasaScreen> {
  ...
  Future<void> _loadData() async {
    ...
    _milestones = await _db.getMilestones('bahasa', _child!.ageInMonths);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFFFF9800), foregroundColor: Colors.white,
        title: Text('Bahasa', style: GoogleFonts.montserrat(fontWeight: FontWeight.w700)), elevation: 0),
      body: _loading ? const Center(child: CircularProgressIndicator(color: AppColors.primary))
          : SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFFF9800).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(14)),
                child: Row(children: [const Icon(Icons.child_care_rounded, color: Color(0xFFFF9800), size: 22), const SizedBox(width: 10),
                  Text('${_child?.name ?? ''} • ${_child?.ageInMonths ?? 0} bulan', style: GoogleFonts.nunito(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFFFF9800)))])),
              const SizedBox(height: 20),
              Text('Perkembangan Bahasa', style: GoogleFonts.montserrat(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textDark)),
              const SizedBox(height: 4),
              Text('Kemampuan berkomunikasi dan memahami bahasa', style: GoogleFonts.nunito(fontSize: 12, color: AppColors.textMuted)),
              const SizedBox(height: 10),
              CustomProgressBar(label: 'Progress', percentage: _progress, color: const Color(0xFFFF9800)),
